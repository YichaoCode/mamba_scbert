...
#)
logging.info("Mamba模型构建完成")

model.to(device)
model = DDP(model, device_ids=[local_rank], output_device=local_rank)
# model = model(x)

logging.info("Mamba分布式模型构建完成")

# optimizer
optimizer = Adam(model.parameters(), lr=LEARNING_RATE)

logging.info("激活函数构建完成")
...
logging.info("CosineAnnealingWarmupRestarts构建完成")
loss_fn = nn.CrossEntropyLoss(ignore_index = PAD_TOKEN_ID, reduction='mean').to(local_rank)
logging.info("CrossEntropyLoss构建完成")
softmax = nn.Softmax(dim=-1)
logging.info("Softmax构建完成")


writer = SummaryWriter("../loss/"+f'%(asctime)s')

logging.info("TensorBoard输出构建完成")

dist.barrier()
# logging.info("dist.barrier()")
for i in range(1, EPOCHS+1):
    logging.info(f"第{i}个epoch")
    train_loader.sampler.set_epoch(i)
    logging.info(f"train_loader.sampler.set_epoch({i})")
    model.train()
    ...
    for index, data in enumerate(train_loader):
        ...
        if index % GRADIENT_ACCUMULATION != 0:
            logging.info("index % GRADIENT_ACCUMULATION != 0")
            with model.no_sync():
                logging.info("上下文函数")
                # pdb.set_trace()
                print("Model:")
                ...
                print(data)
                logits = model(data)
                logging.info("获取logits前")
                #logits = logits[0]
                logging.info("获取logits--1")
                # pdb.set_trace()
                loss = loss_fn(logits.transpose(1, 2), labels) / GRADIENT_ACCUMULATION
                logging.info("获取loss--1")
                loss.backward()
                logging.info("求导--1")
        if index % GRADIENT_ACCUMULATION == 0:
            logging.info("index % GRADIENT_ACCUMULATION == 0")
            logits = model(data)
            #logits = logits[0]
            logging.info("获取logits--2")
            loss = loss_fn(logits.transpose(1, 2), labels) / GRADIENT_ACCUMULATION
            logging.info("获取loss--2")
            loss.backward()
            logging.info("求导--2")
            torch.nn.utils.clip_grad_norm_(model.parameters(), int(1e2))
            logging.info("torch.nn.utils.clip_grad_norm_")
            optimizer.step()
            logging.info("激活")
        ...
        cum_acc += epoch_acc
        logging.info(f"准备写第{index}步数据到Tensorboard")
    ...
    if i % VALIDATE_EVERY == 0:
        ...
        if is_master:
            ...
            logging.info(f'    ==  Epoch: {i} | Validation Loss: {val_loss:.6f} | Accuracy: {val_acc:6.4f}%  ==')
