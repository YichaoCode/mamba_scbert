...
def get_reduced(tensor, local_rank, target_rank, world_size):
    tensor = tensor.clone()
...
def initialize_settings(args):
    ...
    formatter = logging.Formatter('%(asctime)s - [Process %(process)d] - %(levelname)8s - %(funcName)20s:%(lineno)4d - %(message)s')
    ...
    return device, world_size, is_master, model_name, ckpt_dir, SEED, BATCH_SIZE, CLASS, SEQ_LEN, POS_EMBED_USING, local_rank, EPOCHS, LEARNING_RATE, PAD_TOKEN_ID, GRADIENT_ACCUMULATION
...
def prepare_data(args, SEED, BATCH_SIZE, device, world_size, CLASS):
    ...
    logging.info(f"Training dataset size: {data_train.shape[0]}")
    logging.info(f"Validation dataset size: {data_val.shape[0]}")
...
# 训练循环
def train_loop(args, model, train_loader, val_loader, device, world_size, is_master, local_rank, EPOCHS, LEARNING_RATE, PAD_TOKEN_ID, GRADIENT_ACCUMULATION):
    ...
    for i in range(1, EPOCHS+1):
        ...
        for index, data in enumerate(train_loader):
            index += 1
            logging.info(f"Process {local_rank}, Epoch {i}, Step {index}, Batch Data GPU: {data.device}")
            data = data.to(device)
        ...
        if i % VALIDATE_EVERY == 0:
            validate(model, val_loader, device, world_size, is_master, local_rank, i, PAD_TOKEN_ID)
...
# 验证
def validate(model, val_loader, device, world_size, is_master, local_rank, epoch, PAD_TOKEN_ID):
    model.eval()
    dist.barrier()
    running_loss = 0.0
    running_error = 0.0
...
def main():
    ...
    # 训练循环
    # 调用 train_loop 函数，传入命令行参数 args、模型、训练数据加载器、验证数据加载器、设备、世界大小、是否为主进程、本地进程序号、训练轮数、学习率、填充标记 ID 和梯度累积步数
    train_loop(args, model, train_loader, val_loader, device, world_size, is_master, local_rank, EPOCHS, LEARNING_RATE, PAD_TOKEN_ID, GRADIENT_ACCUMULATION)
