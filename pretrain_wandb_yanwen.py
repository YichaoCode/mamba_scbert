...
class Trainer:
    ...
    def train(self):
        ...
        # 初始化 GradScaler
        scaler = torch.cuda.amp.GradScaler()  # 新增：混合精度缩放器
        ...
        for i in range(1, self.EPOCHS + 1):
            ...
            for index, data in enumerate(self.train_loader):
                ...
                try:
                    ...
                    # 自动混合精度：前向传播
                    with torch.cuda.amp.autocast():  # 新增：AMP 自动混合精度范围
                    ...
                    scaler.scale(loss).backward()  # 新增：缩放后的反向传播
                    
                    if index % self.GRADIENT_ACCUMULATION == 0:
                        # 解除缩放后裁剪梯度
                        scaler.unscale_(self.optimizer)  # 新增：解除缩放以进行梯度操作
                        grad_norm = clip_grad_norm_(self.model.parameters(), max_norm=1.0)  # 新增：梯度裁剪
                        logging.info(f"Gradient norm after clipping: {grad_norm}")

                        # 更新参数
                        scaler.step(self.optimizer)  # 新增：用缩放器更新参数
                        scaler.update()  # 新增：更新动态缩放器
        ...
        if args.model_type == "performer":
            ...
            try:
                ...
                ssm_cfg = {
                    ...
                    "layer": "Mamba2"  # 指定使用 Mamba2
                }

                # 创建配置
                config = MambaConfig(
                    d_model=2560,                      # 模型维度
                    d_intermediate=0,                  # 中间层维度
                    n_layer=64,                        # Transformer 层数
                    vocab_size=50277,                  # 词汇表大小
                    ssm_cfg=ssm_cfg,                   # 将 SSM 配置传入
                    rms_norm=True,                     # 使用 RMS Normalization
                    residual_in_fp32=True,             # 在 fp32 中计算残差
                    fused_add_norm=True,               # 使用融合的加法和归一化操作
                    pad_vocab_size_multiple=8,         # 词汇表大小填充到 8 的倍数
                    tie_embeddings=True                # 共享嵌入层权重
)
