#!/bin/bash
#SBATCH --job-name=Performer_a100_15_3_batch3_48h    # 作业名称
#SBATCH --nodes=15                   # 请求的节点数
#SBATCH --ntasks-per-node=1         # 每个节点上的任务数
#SBATCH --output=Performer_batch3_%j.out # 标准输出文件
#SBATCH --error=Performer_batch3_%j.err  # 标准错误文件
...
srun bash -c "torchrun --nproc_per_node=3 --nnodes=15 --node_rank=\$SLURM_NODEID --master_addr=$MASTER_ADDR --master_port=29500 pretrain_modify.py --ckpt_dir './ckpts_performer/' --batch_size 3 --data_path='./data/panglao_human.h5ad' 2>&1"
