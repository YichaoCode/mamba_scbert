#!/bin/bash
#SBATCH --job-name=Mamba_a100_15_3_batch6_48h_continue15    # 作业名称
#SBATCH --nodes=15                   # 请求的节点数
#SBATCH --ntasks-per-node=1         # 每个节点上的任务数
#SBATCH --output=Mamba_continue15_batch3_%j.out # 标准输出文件
#SBATCH --error=Mamba_continue15_batch3_%j.err  # 标准错误文件
...
srun bash -c "torchrun --nproc_per_node=3 --nnodes=6 --node_rank=\$SLURM_NODEID --master_addr=$MASTER_ADDR --master_port=29500 pretrain_modify_mamba_200.py --batch_size 3 --data_path='./data/panglao_human.h5ad' 2>&1"
